...
async def get_students(current_user: dict = Depends(require_role("teacher"))):
...
async def create_notice(notice: Notice, current_user: dict = Depends(require_role("teacher"))):
    ...
    db.notices.insert_one(notice_data)
    return {"success": True}
...
@app.post("/api/teacher/zoom-link")
async def create_zoom_link(zoom_data: ZoomLink, current_user: dict = Depends(require_role("teacher"))):
    ...
    db.zoom_links.delete_many({})
    db.zoom_links.insert_one(zoom_link)
    return {"success": True}

@app.get("/api/teacher/zoom-link")
...
@app.get("/api/teacher/tests/{test_id}/results")
...
@app.post("/api/teacher/student-records/{student_id}")
...
@app.get("/api/teacher/student-records/{student_id}")
...
@app.get("/api/student/download/{filename}")
async def download_note(filename: str, current_user: dict = Depends(require_auth())):
...
async def submit_test(test_id: str, submission: TestSubmission, current_user: dict = Depends(require_role("student"))):
    try:
        ...
        for i, question in enumerate(test['questions']):
            if str(i) in submission.answers and submission.answers[str(i)] == question.get('correct_answer'):
                score += 1
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*50)
    print("Starting FastAPI Classroom Management System...")
    print("="*50)
